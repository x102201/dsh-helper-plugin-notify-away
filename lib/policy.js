/**
 * Completion-edge and "are you looking" policy. Pure, so the host tests and
 * the browser half can share one definition of when a toast should fire.
 *
...
/**
 * Whether one completion should raise a system notification.
 *
 * Cursor's rule: stay silent only while the user is looking at the session
 * that just finished. Notify when they are away (other window / hidden tab)
 * or when a different session completed.
 *
 ...
 *
 * @param {{ visibilityState?: string, hasFocus?: () => boolean }} doc - `document`.
