...
/** Pending `kind` → config event key. Unknown kinds use `otherWait`. */
...
/**
 * Config event for one pending interaction.
 *
 * @param {object | undefined} interaction - one `pendingInteractions` value.
 * @returns {'approval' | 'question' | 'planReview' | 'otherWait'}
 */
export function waitEvent(interaction) {
  const kind = interaction && typeof interaction.kind === 'string' ? interaction.kind : '';
  return WAIT_EVENT_BY_KIND[kind] ?? 'otherWait';
}

/**
 * Whether this event kind is allowed to toast.
 *
 * Omitted keys stay on: only an explicit `false` in options turns a kind off.
 *
 * @param {object} options - watcher options / resolved config.
 * @param {'completion' | 'approval' | 'question' | 'planReview' | 'otherWait'} event
 * @returns {boolean}
 */
export function isEventEnabled(options, event) {
 ...
 * other instance / hidden tab) or when a different session needs them.
 *
 ...
 * @param {boolean} [options.completion=true] - toast on running→idle.
 ...
 * @param {boolean} [options.approval=true] - pending kind `approval`.
 * @param {boolean} [options.question=true] - pending kind `question`.
 * @param {boolean} [options.planReview=true] - pending kind `plan-review`.
 * @param {boolean} [options.otherWait=true] - any later pending kind.
 ...
 *
 * Helper writes two bits:
 * - `panelVisible` (`window.__dshHelperPanelVisible`): this instance's panel
 *   is the one on screen. False when the user is working in another instance.
 *   WebView2 hide() often leaves `visibilityState` as `visible`.
 * - `appFocused` (`window.__dshHelperAppFocused`): any helper window is the
 *   OS foreground. WebView2's `document.hasFocus()` often stays true after
 *   Alt+Tab, so this bit is the authority when present.
 *
 * Stay silent only while this panel is showing and the helper is in front.
 * Another instance, another app, or a hidden tab all count as away.
 *
 * @param {{ visibilityState?: string, hasFocus?: () => boolean, appFocused?: boolean, panelVisible?: boolean }} doc
 * @returns {boolean} true when the user is not looking at this panel.
 */
